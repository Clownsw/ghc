...
data XXExprGhcRn
  ...
  | ExpandedPat
    {-# UNPACK #-} !(HsExpansion (LPat GhcRn)      -- Original failable source pattern
                                 (HsExpr GhcRn))   -- Expanded expression
                                                   -- See Note [Expanding HsDo with HsExpansion]
...
instance Outputable XXExprGhcRn where
  ...
  ppr (ExpandedPat ex) = whenPprDebug (text "[ExpandedPat]") <+> ppr ex
...
instance Outputable XXExprGhcTc where
  ...
  ppr (ExpansionStmt (HsExpanded stmt e))
    = ifPprDebug (vcat [ text "OriginalStmt" <+> ppr stmt
                       , text "Expansion:" <+> ppr e])
                 (ppr stmt)
               -- stmt is an HsExpansion, we print the original
               -- expression (LHsExpr GhcPs), not the
               -- desugared one (LHsExpr GhcTc).
...
ppr_infix_expr_rn (ExpandedPat _) = Nothing
