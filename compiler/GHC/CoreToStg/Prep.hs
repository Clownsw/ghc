...
let-bound:
  ...
  * `CaseBound`: A (NonRec) case binding `case rhs of x -> { __DEFAULT -> .. }`.
                 Some (string literals, lifted bindings), but not all case-bound
                 things can go to top-level, hence it is similar to, but not the
                 same as `LazyContextFloatable :: FloatInfo` described below.
...
(a) violating Core invariants and (b) changing semantics.
  ...
  * A binding is `TopLvlFloatable` if it is `LazyContextFloatable` and also can
    ...
    NB: You might think this is the same as `CaseBound`, but it is not:
        String literals and lifted, ok-for-spec-eval things are
        `TopLvlFloatable` but are `CaseBound`.

This meaning of "at least" is encoded in `floatsAtLeastAsFarAs`.
All lifted bindings are `TopLvlFloatable`.
